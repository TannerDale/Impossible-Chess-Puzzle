...
def change_required_cell(coin_board, cell):
    """Changes the required cell. Updates and highlights it in Excel."""
    if coin_board.data[cell] == 1:
...
def main():
    ...
    coins = Chessboard(wb, workbook_name, wb['Heads or Tails'], 'coin')
    coin_copy = Chessboard(wb, workbook_name, wb['Heads or Tails'], 'coin')
    coin_copy.__dict__.update(coins.__dict__)
    coin_copy.sheet = wb['Changed Heads or Tails']

    [fill_excel_data(b) for b in Chessboard.boards if b is not coin_copy]

    # Un-comment desired running option
    #program_flip_and_find(bin_board, coins, coin_copy)
    #program_flip_user_find(bin_board, coin_copy)
    #user_flip(bin_board, coins, coin_copy)
